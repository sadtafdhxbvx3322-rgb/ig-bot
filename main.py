...
@app.route('/')
def home():
    return "Ping Pong! Bot is Live & Fixed. 🏓"
...
def run_bot():
    ...
    # --- 1. AI FIX: Using Gemini 1.5 Flash ---
            ...
            for t in threads:
                ...
                if "instagram.com" in text or "youtu" in text:
                    ...
                    if link:
                        ...
                        cl.direct_send("❌ Download Failed.", [uid])
                        ...
                        try:
                            ...
                            if "http" in ai_url:
                                ...
                                if link:
                                    ...
                                    cl.direct_send("❌ Audio extract nahi hua.", [uid])
                            else:
                                cl.direct_send("❌ Song nahi mila.", [uid])
                        except:
                            cl.direct_send("❌ AI Error during search.", [uid])
                    else:
                        cl.direct_send("❌ AI Brain not loaded.", [uid])
                ...
                # --- D. AI CHAT ---
                    ...
                    if model:
                        try:
                            prompt = f"You are a friendly Hinglish AI assistant on Instagram.\nHistory:\n{hist}\nUser: {text}\nReply (Short & Witty):"
                            response = model.generate_content(prompt)
                            reply = response.text
                        except Exception as e:
                            reply = "Abhi thoda busy hu (AI Error)."
                ...
                # 🛑 FIX: Removed 'direct_thread_mark_seen' because it causes crashes
                # Hum bas message padh lenge, blue tick lagana zaroori nahi hai agar wo crash kar raha hai.
